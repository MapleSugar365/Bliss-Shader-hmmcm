...
float cloudVol(in vec3 pos, float maxDistance ){
	...
	float rainyFog = (low_gradientFog * 0.5 + 1.0) * rainStrength;
...
vec4 GetVolumetricFog(
	...
	vec3 SmoothLightColor = mix(LightColor, vec3(0.0), rainStrength);
	vec3 skyLightPhased = AmbientColor;
	vec3 LightSourcePhased = SmoothLightColor;
	...
	LightSourcePhased *= sunPhase;
		...
		vec3 LightSourceColor = SmoothLightColor;
